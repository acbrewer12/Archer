...
def fetch_ngrok_url():
    # Polls local ngrok API to get the public URL
    while True:
        try:
            with urllib.request.urlopen('http://localhost:4040/api/tunnels', timeout=3) as r:
                ...
                for t in tunnels:
                    if t.get('proto') == 'https':
                        public_url['url'] = t['public_url']
                        print(f"[ARCHER] Public URL: {t['public_url']}")
                        break
        except Exception:
            pass
        time.sleep(30)
...
def run_display_server():
    ...
    display_app.run(host='0.0.0.0', port=5001, debug=False, use_reloader=False, threaded=True)
