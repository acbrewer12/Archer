...
Write-Host "  (5-15 minutes depending on distro size)" -ForegroundColor Gray
...
if ($foundVmdk) {
    ...
    Write-Host "  (press Enter to skip - you can copy it manually later)" -ForegroundColor Gray
...
if (Test-Path $VmwareExe) {
    ...
    Write-Host ""
    Write-Host "  Without it, install-archer.ps1 will skip VM setup on PCs without VMware." -ForegroundColor Gray
}
...
foreach ($item in $items) {
    $status = if (Test-Path $item.Path) { "[OK]" } else { "[  ]" }
